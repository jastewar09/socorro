...
!cod$ 
        ...
        if (i_access( diaryfile() )) then
          ...
          case (FD_HYBRID)
            ...
            write(x_unit(diaryfile()),'(/,t8,"Exchange   (native): EXX fraction = ",f5.3)') xct%o%hybrid_mixing
            select case(xct%o%ddf_source)
            case (DDFS_LIBXC)
              select case (mpi_nsgroups())
              case (1)
              ...
              if (xct%o%functional > 0) then
                call xc_f90_func_init(xc_func,xc_info,xct%o%functional,spin_status)
                call xc_f90_info_name(xc_info,str)
                write(x_unit(diaryfile()),'(t8,"Exchange-Correlation (libxc): ",a," fraction = ",f5.3)') &
              ...
              if (xct%o%exchange > 0) then
                call xc_f90_func_init(xc_func,xc_info,xct%o%exchange,spin_status)
                call xc_f90_info_name(xc_info, str)
                write(x_unit(diaryfile()),'(t8,"Exchange    (libxc): ",a," fraction = ",f5.3)') &
                     & trim(str), (1.0_double - xct%o%hybrid_mixing)
                call xc_f90_func_end(xc_func)
              end if
              if (xct%o%correlation > 0) then
          ...
          case (CK_SCREENED)
            ...
            write(x_unit(diaryfile()),'(/,t8,"Orbital screening parameter, aka omega^HF (a0^-1): ",f5.3)') xct%o%omega_orb
            write(x_unit(diaryfile()),'(/,t8,"Semilocal screening parameter, aka omega^PBE (a0^-1): ",f5.3)') xct%o%omega_den
          end select
